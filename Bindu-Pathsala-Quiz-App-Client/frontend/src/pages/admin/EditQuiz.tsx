...
import { quizAPI, questionAPI, sessionAPI, API_URL } from '@/services/api';
import { Quiz, Question, QuizSession } from '@/types';
import { ArrowLeft, Plus, Trash2, Save, Calendar, Clock, Edit, Upload, X } from 'lucide-react';
...
export const EditQuiz: React.FC = () => {
  ...
  // Memoize API base URL to prevent re-renders
  const apiBaseUrl = useMemo(() => API_URL.replace('/api', ''), []);
  ...
  const handleAddQuestion = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (newQuestion.image_file) {
        ...
        formData.append('image', newQuestion.image_file);
  ...
  const handleEditQuiz = async (e: React.FormEvent) => {
    ...
    try {
      await quizAPI.update(quizId!, {
        ...
        time_per_question: editQuizData.time_per_question > 0 ? editQuizData.time_per_question : undefined,
        allowed_time: editQuizData.allowed_time > 0 ? editQuizData.allowed_time : undefined,
      });
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (file) {
                              setNewQuestion({ ...newQuestion, image_file: file });
                            }
                          }}
                          className="hidden"
                          id="image-upload"
                        ...
                        {newQuestion.image_file && (
                          <span className="text-sm text-gray-600">
                            {newQuestion.image_file.name}
                          </span>
                        )}
                      </div>
                      ...
                      {newQuestion.image_file && (
                        <div className="relative inline-block">
                          <img
                            src={URL.createObjectURL(newQuestion.image_file)}
                            alt="Question preview"
                            className="w-48 h-32 object-cover rounded-md border"
                          />
                          <button
                            type="button"
                            onClick={() => setNewQuestion({ ...newQuestion, image_file: null })}
                      ...
                      <div className="flex-1">
                        ...
                        {question.image_path && (
                          <div className="mt-2">
                            <img
                              src={
                                question.image_path.startsWith('/') || 
                                question.image_path.includes('uploads') ||
                                question.image_path.startsWith('http')
                                  ? `${apiBaseUrl}${question.image_path}`
                                  : `${apiBaseUrl}/uploads/${question.image_path}`
                              }
                              alt="Question"
                              className="w-48 h-32 object-cover rounded-md border"
                            />
