...
export const QuizProvider: React.FC<QuizProviderProps> = ({ children }) => {
  ...
  const setQuizData = (quiz: Quiz, questions: Question[], attempt: QuizAttempt) => {
  ...
  const setExpiredQuestion = (questionIndex: number) => {
    dispatch({ type: 'SET_EXPIRED_QUESTION', payload: questionIndex });
  };

  const setTimeRemaining = (time: number | null) => {
    dispatch({ type: 'SET_TIME_REMAINING', payload: time });
  };

  const setQuestionTimeRemaining = (timeOrUpdater: number | null | ((prev: number | null) => number | null)) => {
  ...
  const setQuestionStartTime = (questionIndex: number, startTime: number) => {
    dispatch({ type: 'SET_QUESTION_START_TIME', payload: { questionIndex, startTime } });
  };

  const loadTimingData = (expiredQuestions: number[], questionStartTimes: Record<number, number>) => {
