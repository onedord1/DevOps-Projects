...
impl NotificationHandler {
    ...
    async fn send_notifications(
        ...
        let channels = sqlx::query_as::<sqlx::Postgres, models::NotificationChannel>(
            "SELECT id, org_id, name, channel_type, config, is_active, created_at, updated_at FROM notification_channels WHERE org_id = $1 AND is_active = true"
        )
        .bind(org_id)
        .fetch_all(&self.db)
        .await?;
        ...
        for channel in channels {
            ...
            let send_result = match channel.channel_type {
                ...
                NotificationChannelType::Slack | NotificationChannelType::Discord | NotificationChannelType::MsTeams => {
